...
//  LRC             : 2 chars
//  End             : 2 chars
func (mb *asciiPackager) Encode(pdu *PDUwithSlaveid) (adu []byte, err error) {
	var buf bytes.Buffer

	if _, err = buf.WriteString(asciiStart); err != nil {
		return
	}
	if err = writeHex(&buf, []byte{pdu.SlaveID, pdu.FunctionCode}); err != nil {
		return
	}
	...
	var lrc lrc
	lrc.reset()
	lrc.pushByte(pdu.SlaveID).pushByte(pdu.FunctionCode).pushBytes(pdu.Data)
...
func (mb *asciiPackager) Decode(adu []byte) (pdu *PDUwithSlaveid, err error) {
	pdu = &PDUwithSlaveid{}
	// Slave address
